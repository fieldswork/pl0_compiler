...
{
	char BUFFER; // Buffer for Operation Code DMA

	// Read File; place into text segment
	FILE *ifp = fopen(argv[1], "r");

	// Check for improper file IO
	if (ifp == NULL) 
	{
	...
	// Instruction Counter - counts lines of the program file
	int IC = 0;

	// Get number of lines from IFP
	while (!feof(ifp))
	{
		BUFFER = fgetc(ifp);
	...
	while (haltFlag)
	{
		...
		switch(IR.OP)
		{
			...
			case 5:	// CAL L,M - Call procedure at code index M
				printf("%2d ", (PC - 1) / 3); // Prints out the program count
				strcpy(opCode, "CAL");
				PAS[SP - 1] = base(IR.L, BP, PAS); // Static Link
