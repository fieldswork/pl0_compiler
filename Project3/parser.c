...
int VAR_DECLARATION(lexeme *list, int level)
{
	...
	if (list[currToken].type == varsym)
	{
		do
		{
			...
			{
				// Add the current variable to the symbol table
				addToSymbolTable(2, list[currToken].name, 0, level, numVars+2, 0);
			}
...
int STATEMENT(lexeme *list, int level)
{
	...
	// If the current token type is a begin symbol
	if (list[currToken].type == beginsym)
	{
		do
		{
			currToken++; // Get next token
		...
		if (list[currToken].type != endsym)
		{
			if (list[currToken].type == identsym || list[currToken].type == beginsym || list[currToken].type == ifsym || list[currToken].type == whilesym ||
	...
	// If token type is ifsym
	if (list[currToken].type == ifsym)
	{
		currToken++; // Get next token
		...
		if (list[currToken].type != thensym)
		{
		...
		// If token type is elsesym
		if (list[currToken].type == elsesym)
		{
			int jmpIDX = cIndex;
			emit(7, 0, 0); // JMP
